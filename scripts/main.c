...
uint16_t rpm_to_rad_s(int16_t rpm) {
  float omega = rpm  * 2.0f * 3.14159 / 60.0f ; 
    return (uint16_t)(omega * 100.0f + 32768.0f);
}
#if defined(FEEDBACK_SERIAL_USART2) || defined(FEEDBACK_SERIAL_USART3)
typedef struct{
  uint16_t  start;
  uint16_t speedR_meas;
  uint16_t speedL_meas;
  uint16_t  checksum;
} SerialFeedback;
  ...
  while(1) {
    ...
    // ####### FEEDBACK SERIAL OUT #######
      ...
      if (main_loop_counter % 2 == 0) {    // Send data periodically every 10 ms
        ...
        Feedback.speedR_meas	  = (uint16_t)rpm_to_rad_s(rtY_Right.n_mot);
        Feedback.speedL_meas	  = (uint16_t)rpm_to_rad_s(rtY_Left.n_mot);

        #if defined(FEEDBACK_SERIAL_USART3)
          if(__HAL_DMA_GET_COUNTER(huart3.hdmatx) == 0) {

            Feedback.checksum   = (uint16_t)(Feedback.start ^ Feedback.speedR_meas ^ Feedback.speedL_meas 
                                           );
